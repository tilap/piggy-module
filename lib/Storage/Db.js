...
var Storage = (function (_AbstractStorage) {
  ...
  function Storage(collection) {
    ...
    _get(Object.getPrototypeOf(Storage.prototype), 'constructor', this).call(this, collection);
  }
  ...
  _createClass(Storage, [{
    ...
    value: function get() {
      ...
      return new Promise(function (resolve, reject) {
        _this.collection.find(criteria, options).toArray(function (err, items) {
          if (err) {
    ...
    key: 'getByPage',
    value: function getByPage(criteria) {
      var page = arguments.length <= 1 || arguments[1] === undefined ? 1 : arguments[1];
      var limit = arguments.length <= 2 || arguments[2] === undefined ? 15 : arguments[2];
      var orderby = arguments.length <= 3 || arguments[3] === undefined ? 'id' : arguments[3];
      var order = arguments.length <= 4 || arguments[4] === undefined ? 1 : arguments[4];
    ...
    key: 'insert',
    value: function insert(dataArray) {
      var _this2 = this;
      ...
      return new Promise(function (resolve, reject) {
        _this2.collection.insert(dataArray, function (err, insertResult) {
          if (err) {
            return reject(new Error('Storage error: insert() ' + err.message));
          }
          var insertedDatas = insertResult.ops;
    ...
    key: 'update',
    value: function update(criteria, newValues) {
      var _this3 = this;

      var options = arguments.length <= 2 || arguments[2] === undefined ? { upsert: false, multi: true } : arguments[2];

      criteria = this._prepareCriteria(criteria);
      newValues = this._stripIdCriteria(newValues);
      return new Promise(function (resolve, reject) {
        _this3.collection.update(criteria, { $set: newValues }, options, function (err, updateResult) {
          if (err) {
    ...
    value: function _delete(criteria) {
      ...
      criteria = this._prepareCriteria(criteria);
      return new Promise(function (resolve, reject) {
        _this4.collection.remove(criteria, function (err, deleteResult) {
          if (err) {
            return reject(new Error('Storage error: delete() ' + err.message));
          }
          var affetctedRows = deleteResult.result.n;
          resolve(affetctedRows);
        });
      });
    }
  }, {
    key: '_prepareCriteria',
    value: function _prepareCriteria(criteria) {
      if (criteria._id && criteria._id.constructor === String) {
        criteria._id = (0, _mongodb.ObjectId)(criteria._id);
      }
      return criteria;
    }
  }, {
    key: '_stripIdCriteria',
    value: function _stripIdCriteria(criteria) {
      if (criteria._id) {
        delete criteria._id;
      }
      return criteria;
